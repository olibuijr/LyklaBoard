...
class NlpManager(context: Context) {
    ...
    private val providers = guardedByLock {
        mapOf(
            ...
            LyklabordNlpProvider.ProviderId to ProviderInstanceWrapper(LyklabordNlpProvider(context)),
        )
    }
    ...
    /** Refresh the live Lyklabord personal snapshot after dictionary edits/imports. */
    suspend fun refreshLyklabordPersonalModel() {
        val provider = (providers.withLock { it[LyklabordNlpProvider.ProviderId]?.provider }
            as? LyklabordNlpProvider)
        provider?.refreshPersonalModel()
    }
