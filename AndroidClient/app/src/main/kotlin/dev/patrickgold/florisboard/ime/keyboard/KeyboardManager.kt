...
import dev.patrickgold.florisboard.ime.media.emoji.LyklabordEmojiFrecency
import dev.patrickgold.florisboard.ime.nlp.WordSuggestionCandidate
import dev.patrickgold.florisboard.ime.nlp.PunctuationRule
import dev.patrickgold.florisboard.ime.nlp.SuggestionCandidate
import dev.patrickgold.florisboard.ime.voice.ElevenLabsClient
import dev.patrickgold.florisboard.ime.voice.TtsPlayer
...
class KeyboardManager(context: Context) : InputKeyEventReceiver {
    ...
    fun commitCandidate(candidate: SuggestionCandidate) {
        if (candidate is EmojiSuggestionCandidate) {
            LyklabordEmojiFrecency.record(appContext, candidate.emoji.value)
        }
        scope.launch {
    ...
    /**
     * Speaks [text] through ElevenLabs. Settings' sample action intentionally
     * calls this directly, while [speakCurrentWord] honors the TTS preference.
     */
    fun speak(text: String) {
        val normalizedText = text.trim()
        if (normalizedText.isEmpty()) return
        scope.launch { speakInternal(normalizedText) }
    }

    fun speakCurrentWord() {
        if (!prefs.voice.ttsEnabled.get()) return
        val beforeCursor = editorInstance.run { activeContent.getTextBeforeCursor(512) }.trimEnd()
        val word = beforeCursor.substring(beforeCursor.indexOfLast { it.isWhitespace() } + 1)
        if (word.isNotEmpty()) speak(word)
    }

    private suspend fun speakInternal(text: String) {
        val (primaryKey, fallbackKey) = ElevenLabsClient.resolveKeys(prefs.voice.elevenLabsApiKey.get())
        ElevenLabsClient.synthesize(
            text = text,
            voiceId = prefs.voice.ttsVoiceId.get(),
            primaryKey = primaryKey,
            fallbackKey = fallbackKey,
        ).fold(
            onSuccess = { bytes ->
                runCatching { TtsPlayer.play(appContext, bytes) }
                    .onFailure {
                        withContext(Dispatchers.Main) {
                            appContext.showLongToastSync("Text to speech failed")
                        }
                    }
            },
            onFailure = {
    ...
    private fun handleVoiceInput() {
        if (!prefs.voice.voiceInputEnabled.get()) {
        ...
        val audio = voiceInput.stop()
        if (audio == null) {
            appContext.showLongToastSync("Voice input failed")
            return
        }
        scope.launch {
            val (primaryKey, fallbackKey) = ElevenLabsClient.resolveKeys(prefs.voice.elevenLabsApiKey.get())
            val languageCode = subtypeManager.activeSubtype.primaryLocale.language
            ElevenLabsClient.transcribe(
                audio = audio,
                ...
                onSuccess = { transcript ->
                    withContext(Dispatchers.Main) {
                        editorInstance.commitText(transcript)
                    }
                },
                onFailure = {
                    withContext(Dispatchers.Main) {
    ...
    fun handleHardwareKeyboardSpace() {
        val mode = activeSpacebarMode()
        val candidate = commitSpacebarCandidate(mode)
        ...
        if (shouldInsertSpaceAfterCandidate(mode, candidate)) {
            editorInstance.commitText(KeyCode.SPACE.toChar().toString())
        }
    }

    /**
     * Returns the spacebar mode for the active field. Prediction insertion is disabled for
     * fields where text suggestions are inappropriate (URLs, email, search, and passwords).
     */
    private fun activeSpacebarMode(): SpacebarMode {
        val info = editorInstance.activeInfo
        val variation = info.inputAttributes.variation
        val isGatedField = when (variation) {
            ...
            else -> info.imeOptions.action == ImeOptions.Action.SEARCH
        }
        return if (isGatedField) SpacebarMode.completeCurrentWord else prefs.keyboard.spacebarMode.get()
    }

    private fun commitSpacebarCandidate(mode: SpacebarMode): SuggestionCandidate? {
        val candidate = when (mode) {
            SpacebarMode.completeCurrentWord -> nlpManager.getAutoCommitCandidate()
            SpacebarMode.alwaysInsertPrediction ->
                nlpManager.activeCandidates.firstOrNull { it is WordSuggestionCandidate }
            SpacebarMode.alwaysInsertSpace -> null
        }
        candidate?.let { commitCandidate(it) }
        return candidate
    }

    private fun shouldInsertSpaceAfterCandidate(
        mode: SpacebarMode,
        candidate: SuggestionCandidate?,
    ): Boolean {
        return mode == SpacebarMode.alwaysInsertSpace ||
            candidate == null ||
            subtypeManager.activeSubtype.primaryLocale.supportsAutoSpace
    }
