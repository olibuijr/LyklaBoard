...
abstract class FlorisPreferenceModel : PreferenceModel() {
    ...
    inner class Keyboard {
        ...
        val spacebarMode = enum(
            key = "keyboard__spacebar_mode",
            default = SpacebarMode.completeCurrentWord,
        )
    ...
    inner class Voice {
        val elevenLabsApiKey = string(
            key = "voice__elevenlabs_api_key",
            default = "",
        )
        val voiceInputEnabled = boolean(
            key = "voice__input_enabled",
            default = true,
        )
        val ttsEnabled = boolean(
            key = "voice__tts_enabled",
            default = false,
        )
        val ttsVoiceId = string(
            key = "voice__tts_voice_id",
            default = "FLYmruBNq7Ev0c2dUxLx", // "Rödd" — native Icelandic ElevenLabs voice
        )
        val ttsVoiceName = string(
            key = "voice__tts_voice_name",
            default = "Rödd",
        )
    }
