...
| `safety.jsonl` | 600 real-artifact `preserve` assertions: 400 clean identities + 200 valid-word hard negatives, language-balanced |
...
`safety.jsonl` is generated by `type-eval generate-safety` from `dev.jsonl`
and the shipping lexicon/BÍN artifacts. Unlike ordinary rows, its target is
the typed `typo` field (`"expectation":"preserve"`); `intended` preserves the
source word/context that made the hard negative interesting. The scorecard
gates false auto-applies overall, by language, and by safety category against
`scores/corpus-baseline-v1.json`.
